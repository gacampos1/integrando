...
FFinalmente, vemos los modelos propuestos. Primero sin el ??ndice de democracia como independiente, y luego con ??ste. Los resultados se muestran en la Tabla \ref{regresiones} de la p??gina \pageref{regresiones}.
...
Estos son los lugares donde tenemos informaci??n:
...
@
\end{adjustbox}
\caption{Paises con informaci??n diponible}\label{rawmap}
\end{figure}
